...
.. _advanced-tricks-loop-lista-jogos:
...
.. code-block:: shell
	...
	$ ./mame playch10 -lb | awk 'NR > 1 {print $2}' | sed '$d' > pc10.txt
	$ ./mame playch10 -lb | awk '{print $2}' | sed '1d; $d' > pc10.txt
	$ ./mame playch10 -lb | awk '{print $2}' | tail -n +2 | head -n -1 > pc10.txt
...
.. _advanced-tricks-cria-diversas-configs:
...
encerrar a emulação (pressione :kbd:`Esc`), será criado o arquivo
**cfg/sf2.cfg** com as configurações que fizemos, exemplo:
...
.. code-block:: xml
    ...
    <mameconfig version="10">
        <system name="sf2">
            ...
            <mixer>
                <audio_effects tag=":mono">
                    <effect step="1" type="Filters" lowpass_active="1" fl="6000" />
                    <effect step="2" type="Compressor" mode="0" />
                    <effect step="3" type="Reverb" mode="0" />
                    <effect step="4" type="Equalizer" low_shelf="0" high_shelf="0" f1="80" db1="4.000000" q2="1.010000" db2="2.000000" db3="-2.000000" db4="6.000000" f5="4000" db5="6.000000" />
                </audio_effects>
                <sound_map tag=":mono">
...
Mova este arquivo para a sua área de trabalho, abra-o em um editor de
texto para que possamos transformá-lo em um modelo. Para isso, altere
os itens descritos abaixo:

* Na linha ``<system name="sf2">`` altere para ``<system
  name="altere-me">``;
* Exclua de ``<bgfx>`` até ``</bgfx>``;
* Mantenha todo o resto;
* Salve o arquivo como **base.cfg**.
...
Você deverá ter algo semelhante ao exemplo abaixo:
...
.. code-block:: xml
    ...
    <!-- This file is autogenerated; comments and unknown tags will be stripped -->
    <mameconfig version="10">
        <system name="altere-me">
            <mixer>
                <audio_effects tag=":mono">
                    <effect step="1" type="Filters" lowpass_active="1" fl="6000" ql="0.710000" highpass_active="1" fh="20" qh="0.710000" />
                    <effect step="2" type="Compressor" mode="0" />
                    <effect step="3" type="Reverb" />
                    <effect step="4" type="Equalizer" mode="1" low_shelf="1" high_shelf="0" f1="80" db1="6.000000" f2="200" q2="2.110000" db2="5.000000" db3="-4.000000" db4="6.000000" f5="6000" q5="1.510000" db5="9.000000" />
                </audio_effects>
                <sound_map tag=":mono">
                    <node_mapping node="" db="0.000000" />
                </sound_map>
...
O valor de ``system name`` foi alterado para que o nome da ROM possa ser
substituido posteriormente. Sem isso, o MAME identifica o arquivo como
inválido, ignora as nossas configurações e redefine a configuração
predefinida caso o valor de ``system name`` e o nome do arquivo sejam
diferentes do nome da ROM.

Precisamos de uma lista de jogos CPS1, use as informações do capítulo
:ref:`advanced-tricks-loop-lista-jogos`. Para o nosso exemplo, usaremos
o comando a seguir:

.. code-block:: shell

	$ ./mame sf2 -lb | awk 'NR > 1 {print $2}' | sed '$d' > cps1-lista.txt

Confirme se a lista com o nome das ROMs está correta. Já no comando
a seguir nós criaremos os arquivos de configuração e o termo
**"altere-me"** será substituído com o nome da ROM:

.. code-block:: shell

	mkdir cfg && while read -r lista; do sed "s/altere-me/$lista/g" base.cfg > "cfg/$lista.cfg"; done < cps1-lista.txt

Ao final do comando, teremos uma pasta **cfg** com vários arquivos
***.cfg** cujos nomes correspondem aos das ROMs. Ao abrirmos o
**sf2.cfg** em um editor de texto, veremos o seguinte:
...
Para testar, pegue o arquivo **"sf2.cfg"** criado e coloque-o dentro da
pasta **cfg** do seu MAME. Ao iniciar o jogo *Street Fighter II* (**mame
sf2**), você notará que a equalização estará configurada e permanecerá
assim, independentemente de quantas vezes você entre e saia do jogo, a
não ser que você mesmo altere alguma coisa.

Depois de confirmar que está tudo funcionando corretamente, faça o
backup da pasta cfg do MAME e cole todos os arquivos ***.cfg** dentro
dela.
